package.preload['nvim-tetris.aniseed.fennel.repl'] = package.preload['nvim-tetris.aniseed.fennel.repl']
    or function(...)
        local utils = require 'nvim-tetris.aniseed.fennel.utils'
        local parser = require 'nvim-tetris.aniseed.fennel.parser'
        local compiler = require 'nvim-tetris.aniseed.fennel.compiler'
        local specials = require 'nvim-tetris.aniseed.fennel.specials'
        ...
        local save_source = table.concat(
            {
                'local ___i___ = 1',
                'while true do',
...
package.preload['nvim-tetris.aniseed.fennel.view'] = package.preload['nvim-tetris.aniseed.fennel.view']
    or function(...)
...
package.preload['nvim-tetris.aniseed.fennel.specials'] = package.preload['nvim-tetris.aniseed.fennel.specials']
    or function(...)
        local utils = require 'nvim-tetris.aniseed.fennel.utils'
        local view = require 'nvim-tetris.aniseed.fennel.view'
        local parser = require 'nvim-tetris.aniseed.fennel.parser'
        local compiler = require 'nvim-tetris.aniseed.fennel.compiler'
        ...
        SPECIALS['do'] = function(ast, scope, parent, opts, start, chunk, sub_scope, pre_syms)
            ...
            local function compile_body(outer_target, outer_tail, outer_retexprs)
                if len < start0 then
                    ...
                    for i = start0, len do
                        local subopts = { nval = (((i ~= len) and 0) or opts.nval), tail = (((i == len) and outer_tail) or nil), target = (
                                ((i == len) and outer_target) or nil
                            ) }
...
(tbl:method-name ...) instead.]]
        ...
        local function compiler_env_warn(_, key)
            ...
            if v and io and io.stderr and not already_warned_3f[key] then
                ...
                (io.stderr):write(compile_env_warning:format('use global', key))
            end
            return v
        end
        ...
        local safe_compiler_env = setmetatable(
            {
                assert = assert,
                bit = rawget(_G, 'bit'),
                error = error,
                getmetatable = safe_getmetatable,
                ipairs = ipairs,
        ...
        local function compiler_env_domodule(modname, env, _3fast, _3fscope)
            ...
            return utils['fennel-module'].dofile(
                filename,
                { allowedGlobals = globals, env = env, scope = scope, useMetadata = utils.root.options.useMetadata },
                modname,
                filename
            )
        end
        ...
        local function metadata_only_fennel(modname)
            if
                (modname == 'nvim-tetris.aniseed.fennel.macros')
                or (package and package.loaded and ('table' == type(package.loaded[modname])) and (package.loaded[modname].metadata == compiler.metadata))
            then
...
Consider using import-macros instead as it is more flexible.]]
        ...
        return {
            ['current-global-names'] = current_global_names,
            ['load-code'] = load_code,
            ['macro-loaded'] = macro_loaded,
            ['make-compiler-env'] = make_compiler_env,
            ['make-searcher'] = make_searcher,
            ['search-module'] = search_module,
            ['wrap-env'] = wrap_env,
            doc = doc_2a,
        }
    end
package.preload['nvim-tetris.aniseed.fennel.compiler'] = package.preload['nvim-tetris.aniseed.fennel.compiler']
    or function(...)
        local utils = require 'nvim-tetris.aniseed.fennel.utils'
        local parser = require 'nvim-tetris.aniseed.fennel.parser'
        local friend = require 'nvim-tetris.aniseed.fennel.friend'
        local unpack = (table.unpack or _G.unpack)
        local scopes = {}
        local function make_scope(parent)
            ...
            return {
                autogensyms = {},
                depth = _0_,
                hashfn = (parent0 and parent0.hashfn),
                includes = setmetatable({}, { __index = (parent0 and parent0.includes) }),
                macros = setmetatable({}, { __index = (parent0 and parent0.macros) }),
                manglings = setmetatable({}, { __index = (parent0 and parent0.manglings) }),
                parent = parent0,
                refedglobals = setmetatable({}, { __index = (parent0 and parent0.refedglobals) }),
                specials = setmetatable({}, { __index = (parent0 and parent0.specials) }),
                symmeta = setmetatable({}, { __index = (parent0 and parent0.symmeta) }),
                unmanglings = setmetatable({}, { __index = (parent0 and parent0.unmanglings) }),
                vararg = (parent0 and parent0.vararg),
            }
        end
        ...
        local serialize_subst = { ['\v'] = '\\v', ['\f'] = '\\f', ['\a'] = '\\a', ['\b'] = '\\b', ['\t'] = '\\t', ['\n'] = 'n' }
        ...
        local function do_quote(form, scope, parent, runtime_3f)
            ...
            elseif utils['list?'](form) then
                ...
                return string.format(
                    ('setmetatable({filename=%s, line=%s, bytestart=%s, %s}' .. ', getmetatable(list()))'),
                    filename,
                    (form.line or 'nil'),
                    (form.bytestart or 'nil'),
                    mixed_concat(mapped, ', ')
                )
            elseif utils['sequence?'](form) then
                ...
                return string.format(
                    'setmetatable({%s}, {filename=%s, line=%s, sequence=%s})',
                    mixed_concat(mapped, ', '),
                    filename,
                    _1_,
        ...
        return {
            ['apply-manglings'] = apply_manglings,
            ['compile-stream'] = compile_stream,
            ['compile-string'] = compile_string,
            ['declare-local'] = declare_local,
            ['do-quote'] = do_quote,
            ['global-mangling'] = global_mangling,
            ['global-unmangling'] = global_unmangling,
            ['keep-side-effects'] = keep_side_effects,
            ['make-scope'] = make_scope,
            ['require-include'] = require_include,
            ['symbol-to-expression'] = symbol_to_expression,
            assert = assert_compile,
            autogensym = autogensym,
            compile = compile,
            compile1 = compile1,
            destructure = destructure,
            emit = emit,
            gensym = gensym,
            macroexpand = macroexpand_2a,
            metadata = make_metadata(),
            scopes = scopes,
            traceback = traceback,
        }
    end
package.preload['nvim-tetris.aniseed.fennel.friend'] = package.preload['nvim-tetris.aniseed.fennel.friend']
    or function(...)
        local function ast_source(ast)
            local m = getmetatable(ast)
            return ((m and m.line and m) or (('table' == type(ast)) and ast) or {})
        ...
        local suggestions = {
            ...
            ["can't start multisym segment with a digit"] = { 'removing the digit', 'adding a non-digit before the digit' },
            ['cannot call literal value'] = { 'checking for typos', 'checking for a missing function name' },
            ...
            ['expected a function.* to call'] = { 'removing the empty parentheses', 'using square brackets if you want an empty table' },
            ...
            ['expected even number of values in table literal'] = { 'removing a key', 'adding a value' },
            ['expected local'] = { 'looking for a typo', 'looking for a local which is used out of its scope' },
            ...
            ['expected var (.*)'] = { 'declaring %s using var instead of let/local', 'introducing a new local instead of changing the value of %s' },
            ...
            ['illegal character: (.)'] = { 'deleting or replacing %s', 'avoiding reserved characters like ", \\, \', ~, ;, @, `, and comma' },
            ...
            ['mismatched closing delimiter (.), expected (.)'] = { 'replacing %s with %s', 'deleting %s', 'adding matching opening delimiter earlier' },
            ...
            ['unexpected closing delimiter (.)'] = { 'deleting %s', 'adding matching opening delimiter earlier' },
            ...
            ['unused local (.*)'] = { 'fixing a typo so %s is used', 'renaming the local to _%s' },
            ['use of global (.*) is aliased by a local'] = { 'renaming local %s', 'refer to the global using _G.%s instead of directly' },
        }
        local unpack = (table.unpack or _G.unpack)
        ...
        local function friendly_msg(msg, _0_0, source)
            ...
            if ok and codeline and bytestart and byteend then
                table.insert(
                    out,
                    (string.rep(' ', (bytestart - bol - 1)) .. '^' .. string.rep('^', math.min((byteend - bytestart), ((bol + #codeline) - bytestart))))
                )
            end
        ...
        local function parse_error(msg, filename, line, bytestart, source)
            return error(
                friendly_msg(('Parse error in %s:%s\n  %s'):format(filename, line, msg), { bytestart = bytestart, filename = filename, line = line }, source),
                0
            )
        end
        return { ['assert-compile'] = assert_compile, ['parse-error'] = parse_error }
    end
package.preload['nvim-tetris.aniseed.fennel.parser'] = package.preload['nvim-tetris.aniseed.fennel.parser']
    or function(...)
        local utils = require 'nvim-tetris.aniseed.fennel.utils'
        local friend = require 'nvim-tetris.aniseed.fennel.friend'
        local unpack = (table.unpack or _G.unpack)
            ...
            local function parse_stream()
                ...
                local function check_malformed_sym(rawstr)
                    ...
                    elseif rawstr:match '%.[0-9]' then
                        return parse_error(
                            ("can't start multisym segment " .. 'with a digit: ' .. rawstr),
                            (((byteindex - #rawstr) + rawstr:find '%.[0-9]') + 1)
                        )
...
local utils = nil
package.preload['nvim-tetris.aniseed.fennel.utils'] = package.preload['nvim-tetris.aniseed.fennel.utils']
    or function(...)
        ...
        local function multi_sym_3f(str)
            ...
            elseif type(str) ~= 'string' then
                ...
                return (
                        (#parts > 0)
                        and (str:match '%.' or str:match ':')
                        and not str:match '%.%.'
                        and (str:byte() ~= string.byte '.')
                        and (str:byte(-1) ~= string.byte '.')
                        and parts
                    )
            end
        end
        ...
        return {
            ['comment?'] = comment_3f,
            ['debug-on?'] = debug_on_3f,
            ['expr?'] = expr_3f,
            ['list?'] = list_3f,
            ['lua-keywords'] = lua_keywords,
            ['member?'] = member_3f,
            ['multi-sym?'] = multi_sym_3f,
            ['propagate-options'] = propagate_options,
            ['quoted?'] = quoted_3f,
            ['sequence?'] = sequence_3f,
            ['sym?'] = sym_3f,
            ['table?'] = table_3f,
            ['valid-lua-identifier?'] = valid_lua_identifier_3f,
            ['varg?'] = varg_3f,
            ['walk-tree'] = walk_tree,
            allpairs = allpairs,
            comment = comment_2a,
...
utils = require 'nvim-tetris.aniseed.fennel.utils'
local parser = require 'nvim-tetris.aniseed.fennel.parser'
local compiler = require 'nvim-tetris.aniseed.fennel.compiler'
local specials = require 'nvim-tetris.aniseed.fennel.specials'
local repl = require 'nvim-tetris.aniseed.fennel.repl'
local view = require 'nvim-tetris.aniseed.fennel.view'
...
do
  ...
  {:-> ->* :->> ->>* :-?> -?>* :-?>> -?>>* :?. ?dot
   ...
   :match match-where}
    ...
    local module_name = 'nvim-tetris.aniseed.fennel.macros'
    local _ = nil
    local function _0_()
        return mod
    end
    ...
    local built_ins = eval(
        builtin_macros,
        { allowedGlobals = false, env = env, filename = 'src/fennel/macros.fnl', moduleName = module_name, scope = compiler.scopes.compiler, useMetadata = true }
    )
    for k, v in pairs(built_ins) do
        compiler.scopes.global.macros[k] = v
    end
    compiler.scopes.global.macros['\206'] = compiler.scopes.global.macros.lambda
