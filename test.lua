...
M.defaults = setmetatable(
    { fallback = { exception_breakpoints = 'default', stepping_granularity = 'statement', terminal_win_cmd = 'belowright new', focus_terminal = false } },
    {
        __index = function(tbl, key)
            tbl[key] = {}
            return rawget(tbl, key)
        ...
        __newindex = function(tbl, key)
            rawset(tbl, key, setmetatable({}, { __index = from_fallback }))
        end,
    }
)
...
local function run_adapter(adapter, configuration, opts)
    ...
    opts = vim.tbl_extend('keep', opts, { cwd = options.cwd, env = options.env })
...
local function maybe_enrich_config_and_run(adapter, configuration, opts)
    ...
    assert(adapter.type, 'Adapter for ' .. configuration.type .. ' must have the `type` property set to `executable` or `server`')
    if adapter.enrich_config then
        assert(type(adapter.enrich_config) == 'function', '`enrich_config` property of adapter must be a function: ' .. vim.inspect(adapter))
...
local function select_config_and_run()
    ...
    assert(
        ...
        string.format('`dap.configurations.%s` must be a list of configurations, got %s', filetype, vim.inspect(configurations))
    )
    ...
    lazy.ui.pick_if_many(configurations, 'Configuration: ', function(i)
        return i.name
...
function M.run(config, opts)
    ...
    opts = opts or {}
    last_run = { config = config, opts = opts }
    if opts.before then
    ...
    elseif type(adapter) == 'function' then
        ...
        utils.notify(string.format('Invalid adapter `%s` for config `%s`', vim.inspect(adapter), config.type), vim.log.levels.ERROR)
    end
end
function M.run_last()
    if last_run then
...
function M.toggle_breakpoint(condition, hit_condition, log_message, replace_old)
    lazy.breakpoints.toggle { condition = condition, hit_condition = hit_condition, log_message = log_message, replace = replace_old }
    ...
    local function restore_breakpoints()
        ...
        for buf, buf_bps in pairs(bps) do
            for _, bp in pairs(buf_bps) do
                local line = bp.line
                local opts = { condition = bp.condition, log_message = bp.logMessage, hit_condition = bp.hitCondition }
...
function M.continue()
    ...
    elseif session.stopped_thread_id then
        ...
        local prompt = (session.initialized and 'Session active, but not stopped at breakpoint> ' or 'Session still initializing> ')
        local choices = {
            { label = 'Terminate session', action = M.terminate },
            { label = 'Pause a thread', action = M.pause },
            { label = 'Restart session', action = M.restart },
            {
...
function M.attach(adapter, config, opts, bwc_dummy)
    if type(adapter) == 'string' then
        utils.notify('dap.launch signature changed from (host, port, config) to (adapter, config), please adjust', vim.log.levels.WARN)
        local host = adapter
        local port = config
        config = opts
        opts = bwc_dummy
